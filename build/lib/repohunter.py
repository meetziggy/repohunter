...
if not GHTOK:
    # Unauthenticated GitHub is 60 req/hr and 403s mid-run, which shows up as bogus
    # "could not resolve" and zero-file scans. Borrow the gh CLI's token when it's there.
    try:
        GHTOK = subprocess.check_output(["gh", "auth", "token"], text=True,
...
def llm(system, prompt, timeout=180):
    """One chat completion → text. Tries llm.backend, then llm.fallback. Never raises."""
    b = CFG["llm"]
...
def _llm_once(backend, b, system, prompt, timeout):
    if backend == "codex-cli":
        # Routes through the ChatGPT subscription via the Codex CLI instead of a metered API key.
        # --ephemeral keeps no session files; read-only sandbox because we only want text back.
        import tempfile
        out_path = None
        try:
            fd, out_path = tempfile.mkstemp(suffix=".txt"); os.close(fd)
            cmd = ["codex", "exec", "--skip-git-repo-check", "--ephemeral",
                   "-s", "read-only", "--color", "never", "-o", out_path]
            if b.get("model"):
                cmd += ["-m", b["model"]]
            cmd.append("-")
            subprocess.run(cmd, input="%s\n\n%s" % (system, prompt),
...
EXAMPLE_SECRETS = {"AKIAIOSFODNN7EXAMPLE", "AKIAI44QH8DHBEXAMPLE"}
...
# ── Skills: ship the instinct, not just the tools ─────────────────────────────
# The MCP server gives an agent RepoHunter's tools. A skill gives it the reflex to
# reach for them unprompted. Both ship together (see .claude-plugin/), but agents
# that aren't Claude Code need the folder copied into their own skills directory.
SKILL_DIRS = ("repohunter", "repo-intake")


def _skills_root():
    """Where the bundled skills live. Works from a clone and from an sdist install."""
    for cand in (os.path.join(os.getcwd(), "skills"), os.path.join(HERE, "skills")):
        if os.path.isdir(cand):
            return cand
    return None


def install_skill(dest=None, which=None):
    """Copy bundled skills into an agent's skills directory.

    Default destination is ~/.claude/skills (Claude Code). Note that Claude Code's
    store and the claude.ai profile store do NOT sync — installing here does not make
    the skill visible in chat, and vice versa.
    """
    import shutil
    src_root = _skills_root()
    if not src_root:
        sys.stderr.write("error: no bundled skills/ directory found next to repohunter.py "
                         "or in the current directory.\n")
        return 2
    dest = dest or os.path.join(os.path.expanduser("~"), ".claude", "skills")
    wanted = [which] if which else list(SKILL_DIRS)
    installed = []
    for name in wanted:
        src = os.path.join(src_root, name)
        if not os.path.isdir(src):
            sys.stderr.write("skip %s — not found in %s\n" % (name, src_root))
            continue
        if not os.path.exists(os.path.join(src, "SKILL.md")):
            sys.stderr.write("skip %s — no SKILL.md\n" % name)
            continue
        ...
        try:
            ...
            sys.stderr.write("error installing %s: %s\n" % (name, e))
            return 1
    if not installed:
        sys.stderr.write("error: nothing installed.\n")
        return 2
    for t in installed:
        print("installed %s" % t)
    print("\nRestart your agent to pick them up. The tools are separate — add the MCP "
          "server too:\n  claude mcp add repohunter -- uvx repohunter-mcp")
    return 0
...
USAGE = """RepoHunter — reuse, don't reinvent.
  ...
  repohunter install-skill [dir]         copy the bundled skills into ~/.claude/skills
...
def main(argv=None):
    ...
    fn = {"evaluate": lambda: evaluate(a[0]), "plan": lambda: plan_mode(a[0]),
          ...
          "serve": serve, "refresh": refresh, "profiles": profiles_mode,
          "install-skill": lambda: install_skill(
              next((x for x in a if not x.startswith("-")), None))}.get(cmd)
    if fn is None:
